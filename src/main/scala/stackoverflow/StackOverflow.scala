...
object StackOverflow extends StackOverflow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    raw.foreach(posting => {
    //      println(posting.parentId+" "+posting.id)
    //    })
...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val questionPart = questions.partitionBy(partitioner).persist()
    val answerPart = answers.partitionBy(partitioner).persist()

    questionPart.join(answerPart).partitionBy(partitioner).persist().groupByKey().cache()
  }
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    def returnPair(value: Iterable[(Question, Answer)]): (Question, Int) = {

      var listBuffer = ListBuffer[Answer]()
      var setQuestion = mutable.HashSet[Question]()
      value.foreach(line => {
        setQuestion.+=(line._1)
        listBuffer.+=(line._2)
      })
      (setQuestion.head, answerHighScore(listBuffer.toArray))
    }

    val groupedJoinFormatted = grouped.map(x => {
      (returnPair(x._2))
    })

    groupedJoinFormatted.cache()
  }
